...
[points, faces, groups] = point2mesh_group(double(XYZvalid));
...
groups = groups';
groups = groups + 2;
[groupCount, groupMap] = hist(groups,unique(groups));

groupCountMap = zeros(max(groups),1);
for i = 1:length(groupCount)
    groupCountMap(int32(groupMap(i))) = groupCount(i);
end

clusterThres = 1000;

points(groupCountMap(groups) < clusterThres, 1) = 0;
points(groupCountMap(groups) < clusterThres, 2) = 0;
points(groupCountMap(groups) < clusterThres, 3) = 0;

faces(points(int32(faces(:,1))) == 0 | points(int32(faces(:,2))) == 0 | points(int32(faces(:,3))) == 0, :) = [];
...
write_img([folderPath,'/scene',num2str(imageId),'_clean_',num2str(clusterThres),'.jpeg'],allV,allF,imageData);
write_ply([folderPath,'/scene',num2str(imageId),'_clean_',num2str(clusterThres),'.ply'],allV,allF,imageData);

% end
%
%
% write_img('test.png',points,faces,imageData);
% write_ply('test.ply',points,faces,imageData);
end