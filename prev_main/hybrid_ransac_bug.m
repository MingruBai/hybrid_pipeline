...
for i = 1:length(objDataset)
    ...
    [vList,fList] = create_obj(objData, ['/volumes/sunhome/Yinda_2303/',bestPath]);
...
inside_bb_matrix = reshape(inside_bb, [530,730]);
...
points2ply([folderPath,'/scene',num2str(imageId),'.ply'], points3dall');
imwrite(depthNorm, [folderPath,'/scene',num2str(imageId),'.jpeg']);
...
[rgb,points3dall, XYZ]=read_3d_pts_general(hybridDepth,imageData.K,size(hybridDepth),imageData.rgbpath);
points3dall = (imageData.Rtilt*points3dall')';
points2ply([folderPath,'/scene',num2str(imageId),'_all.ply'], points3dall');
imwrite(depthNorm, [folderPath,'/scene',num2str(imageId),'_all.jpeg']);

%only consider wall points inside bb:
roomDepth(~inside_bb_matrix) = 0;
hybridDepth = render_depth(allV, allF, imageData);
hybridDepth(hybridDepth == 0) = roomDepth(hybridDepth == 0);
depthMin = min(hybridDepth(:));
depthMax = max(hybridDepth(:));
